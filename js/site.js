/* Ana Eugenia Ferreira — behaviour.

   Two rules hold throughout:
   1. Every element's resting state is visible. Start states live behind
      .js-motion, which is only ever added by this file. A script that fails
      to parse costs an animation, never content.
   2. .js-motion is armed only while the tab is visible. CSS transitions and
      rAF are both suspended in a hidden tab, so arming early would leave a
      crawler or a background tab looking at a clipped, transparent page. */
(function () {
  'use strict';
  ...
  /* ---------- hero entrance ----------
     Timers, not requestAnimationFrame: rAF is throttled to zero in a
     background tab and the hero would never uncover. */
  ...
  var targets = [].slice.call(document.querySelectorAll('.rv, .spread, .overlay, .quote-plate, .ledger'));
  if (targets.length) {
  ...
  /* ---------- the five languages ---------- */
  var tongues = document.querySelector('[data-tongues]');
  if (tongues) {
    var tio = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) { e.target.classList.add('is-lit'); tio.unobserve(e.target); }
      });
    }, { threshold: 0.4 });
    tio.observe(tongues);
    window.setTimeout(function () { tongues.classList.add('is-lit'); }, 5000);
  }
